...
def technicalevent(request):
    if request.method == "POST":
        ...
        technicalevent = EventAdvisor(name=name, email=email, phone=phone, destination=destination, date=date,
                                      time=time, guest=guest, desc=desc)
        technicalevent.save()
        return render(request, 'payment.html')
...
def eventadvisor(request):
    if request.method == "POST":
        ...
        time = request.POST.get('time')
        guest = request.POST.get('guest')
        eventadvisor = EventAdvisor(name=name, email=email, phone=phone, destination=destination, date=date, time=time,
                                    guest=guest, desc=desc)
        eventadvisor.save()
        return render(request, 'payment.html')
...
def col(request):
    return render(request, 'collegevent.html')